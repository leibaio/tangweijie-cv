  ...
  zh: [
    {
      ...
      bulletPoints: [
        ...
        "MongoDB 配置对比工具：多环境（test/trial/prod）MongoDB 配置可视化对比，支持智能抽样对比大表（牌库表前/中/后各10条）、自动过滤备份集合、正则匹配差异字段，效率提升 10 倍+",
        "配置表 Excel 导入：基于阿里 EasyExcel 的配置表导入解析，支持房间配置、杂项配置等多类型，自动校验字段类型和格式，减少运营手工配置错误",
        "快速发布系统：AOP 切面统一管理配置发布版本号，支持多模块（活动/比赛/渠道等）独立发布，解耦业务逻辑与版本管理，发布过程可追溯",
  ...
  en: [
    {
      ...
      bulletPoints: [
        ...
        "MongoDB Config Compare: Multi-environment (test/trial/prod) MongoDB config visualization, smart sampling for large tables (front/middle/back 10 samples for poker tables), auto-filter backup collections, regex-based field diff detection, 10x+ efficiency improvement",
        "Config Excel Import: Based on Alibaba EasyExcel, supports room config, sundry config and other types, auto-validation of field types and formats, reducing manual config errors",
        "Quick Publish System: AOP-based unified version management for multi-module publishing (activity/tournament/channel), decoupled business logic and version management, traceable publish process",
      ],
      tags: [
        { text: "Vue3", color: "blue" },
        { text: "TypeScript", color: "blue" },
